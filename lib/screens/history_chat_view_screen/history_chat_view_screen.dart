...
class HistoryChatViewScreen extends StatefulWidget {
  ...
  HistoryChatViewScreen(
      {Key? key, this.answer, this.question, required this.historyPage})
      : super(key: key);
...
class _HistoryChatViewScreenState extends State<HistoryChatViewScreen> {
  ...
  dynamic chatComplete(String question) async {
    ...
    try {
      final request = ChatCompleteText(messages: [
        Map.of({"role": "user", "content": question.trim()})
      ], maxToken: token, model: ChatModel.gptTurbo);
      ...
      final request = CompleteText(
          prompt: question, model: Model.textDavinci3, maxTokens: token);
      final response = await openAI.onCompletion(request: request);
      String answer = response?.choices.last.text.trim() ?? "";
      data = answer;
      setState(() {});
  ...
  void sendMessageToAPI(String question) async {
    ...
    try {
      ...
      await SharedPrefsUtils.storeChat(
          ...
          sentByMe: false,
          dateTime: "$day/$month/$year",
          answer: 'Không nhận được phản hồi vui lòng thử lại');
      addMessageToMessageList("Không nhận được phản hồi vui lòng thử lại", false);
      isVoiceOn == true
          ? _speak("Không nhận được phản hồi vui lòng thử lại")
          : null;
    }
  ...
  //   } catch (e) {
  //     await SharedPrefsUtils.storeChat(chat: messageController.text , sentByMe: false,dateTime: "$day/$month/$year",answer: 'Không nhận được phản hồi vui lòng thử lại');
  //     addMessageToMessageList("Không nhận được phản hồi vui lòng thử lại", false);
  //   }
  //
  //   setState(() {
